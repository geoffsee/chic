...
import { FALLBACK_CATALOG } from "./gutenberg/fallbackCatalog";
import { GUTENBERG_LIBRARY_ID, GUTENBERG_LIBRARY_LABEL } from "./gutenberg/constants";
...
const fallbackPage = (libraryId: LibraryId, page: number, search: string): CatalogPage => {
  // Offline fallback is Gutenberg-only today; other libraries surface the error.
  const catalog = libraryId === GUTENBERG_LIBRARY_ID ? FALLBACK_CATALOG : [];
  const filtered = catalog.filter((book) => matchesSearch(book, search));
...
/**
 * Browser client for a single registered library.
 * Talks only to `/api/*` (paginated catalog + chunked text).
 */
export class ApiLibrary extends Library {
  readonly id: LibraryId;
  readonly label: string;

  constructor(id: LibraryId = GUTENBERG_LIBRARY_ID, label: string = GUTENBERG_LIBRARY_LABEL) {
    super();
    this.id = id;
    this.label = label;
  }
  ...
  async listBooks(options?: ListBooksOptions): Promise<CatalogPage> {
    ...
    try {
      ...
      if (page === 1) {
        ...
        return fallbackPage(this.id, page, search);
      }
  ...
  async fetchBookTextPage(
    ...
    const response = await fetchWithTimeout("/api/book-text", TEXT_FETCH_TIMEOUT_MS, {
      ...
      body: JSON.stringify({
        id: book.id,
        libraryId: book.libraryId || this.id,
    ...
    return {
      ...payload,
      libraryId: payload.libraryId || this.id,
    };
  }
}
